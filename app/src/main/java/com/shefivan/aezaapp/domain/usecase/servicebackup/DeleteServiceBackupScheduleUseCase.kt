...
class DeleteServiceBackupScheduleUseCase @Inject constructor(
    private val repository: ServiceBackupRepository,
    private val errors: AppErrorEmitter,
) {
    suspend operator fun invoke(serviceId: Long) = errors.safeApiCall { repository.deleteSchedule(serviceId) }
}
